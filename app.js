...
const BANNED_IDS = new Set([
  ...
  'aghanims_shard',
  // Формально полноценные предметы, но по факту почти всегда — просто
  // промежуточный компонент на пути к чему-то большему (никто их не
  // оставляет "как есть" осознанно). Это игровое знание, не выводится
  // из полей датасета, поэтому баним точечно, а не общим правилом —
  // общее правило зацепило бы и Sange/Kaya/Helm of the Dominator/Shadow
  // Blade, которые как раз часто оставляют финальным предметом в слоте.
  'oblivion_staff', // → Orchid Malevolence / Bloodthorn
  'pers',           // → Perseverance → Octarine Core / Bloodstone
  'soul_booster',   // → Octarine Core / Bloodstone
]);
